...
class Lowrate extends StatefulWidget {
  const Lowrate({super.key});

  @override
  State<Lowrate> createState() => _HeartrateState2();
}

class _HeartrateState2 extends State<Lowrate> {
  ...
  Widget build(BuildContext context) {
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => Lowhearthome()), // Navigate to Heartrate1
    );
  },
),
        title: Text(
        ...
        GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => DateDetailsPage(date: currentDay),
              ),
            );
          },
          child: Padding(
...
class DateDetailsPage2 extends StatelessWidget {
  final DateTime date;

  const DateDetailsPage2({required this.date, super.key});
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (context) => DateDetailsPage(date: date.subtract(Duration(days: 1))),
                        ),
                      );
                    },
                  ),
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (context) => DateDetailsPage(date: date.add(Duration(days: 1))),
                        ),
                      );
                    },
                  ),
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        _buildIndicator("LOW",const Color.fromARGB(255, 255, 132, 0), ),
                        SizedBox(width: 20),
                        _buildIndicator("NORMAL", Colors.white),
                        SizedBox(width: 20),
                        _buildIndicator("HIGH", Colors.white),
                      ],
                    ),
                    SizedBox(height: screenHeight * 0.02),
...
// Function to build low BPM info section
Widget _buildHealthyHeartInfo(double screenWidth) {
  return Container(
    ...
    decoration: BoxDecoration(
      borderRadius: BorderRadius.circular(15),
      color: const Color.fromARGB(255, 241, 127, 6), // Red to indicate an issue
      boxShadow: [
        BoxShadow(
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 10),
              _buildResultItem("⚠ LOW Heart Rate", LucideIcons.alertTriangle),
              _buildSubItem("• Below 60 BPM (Bradycardia)"),
              _buildSubItem("• Can cause dizziness, fatigue, or fainting"),
              Divider(thickness: 1),
              _buildResultItem("📌 Possible Causes", LucideIcons.info),
              _buildSubItem("  - Medications 💊"),
              _buildSubItem("  - Heart conditions ❤️"),
              _buildSubItem("  - Electrolyte imbalance 🧂"),
              Divider(thickness: 1),
              _buildResultItem("⚠ Risks & Symptoms", LucideIcons.thermometer),
              _buildSubItem("  - Fatigue & dizziness"),
              _buildSubItem("  - Shortness of breath 😮‍💨"),
              _buildSubItem("  - Risk of fainting or cardiac arrest ⚕"),
              Divider(thickness: 1),
              _buildResultItem("📊 Monitoring & Actions", LucideIcons.stethoscope),
              _buildSubItem("  - Check with a doctor 👩‍⚕️"),
              _buildSubItem("  - Increase activity levels 🏃‍♂️"),
              _buildSubItem("  - Stay hydrated & eat a balanced diet 🍽"),
            ],
          ),
        ),
      ],
